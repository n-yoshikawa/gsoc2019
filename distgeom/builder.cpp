...
namespace OpenBabel
{
  ...
  **/
  ...
  bool OBBuilder::Build(OBMol &mol, bool stereoWarnings)
  {
    ...
    OBMol workMol = mol;
    cerr << conv.WriteString(&mol, true) << endl;
    ...
    for(vector<OBMol>::iterator f = fragments.begin(); f != fragments.end(); ++f) {
      ...
      if(!isMatchRigid) {    // if rigid fragment is not in database
        // Run distance geometry for this fragment
	cerr << "Run distance geometry" << endl;
        OBDistanceGeometry dg;
        dg.GetGeometry(*f, false); // Geometry without using current geometry
	cerr << "Finish distance geometry" << endl;

        // Retrieve atomic coordinates in canonical SMILES order
        vector<vector3> coords;
	...
	for (unsigned int index = 0; index < canonical_order.size(); ++index) {
		...
		OBAtom *atom = (*f).GetAtom(order);
		coords.push_back(atom->GetVector());
	}
	cerr << "Finish loading coordinates" << endl;

        // Set the coordinates of atoms in workmol
        OBSmartsPattern sp;
        if (!sp.Init(fragment_smiles)) {
          obErrorLog.ThrowError(__FUNCTION__, " Could not parse SMARTS from fragment", obInfo);
        } else if (sp.Match(mol)) { // for all matches
	    ...
	    cerr << "will set coordinates" << endl;
            // Have any atoms of this match already been added?
            int counter;
            for (k = j->begin(), counter=0; k != j->end(); ++k, ++counter) { // for all atoms of the fragment
              // set coordinates for atoms
